...
document.addEventListener("DOMContentLoaded", async () => {
  if (window.__BUSINESS_DATA__) {
    renderBusinessDataClean(window.__BUSINESS_DATA__);
  } else {
    try {
...
function renderBusinessDataClean(data) {
  ...
  document.querySelectorAll(".city-name-text").forEach((el) => (el.textContent = city));

  // Tagline & Hero Description
  const taglineEl = document.getElementById("hero-title-clean");
  if (taglineEl) {
    taglineEl.innerHTML = `
      <span class="hero-business-highlight brand-name-text" style="display:block; font-size:2.4rem; font-weight:800; color:#FFFFFF; line-height:1.15; margin-bottom:8px; text-shadow:0 3px 14px rgba(0,0,0,0.7);">${bName}</span>
      <span class="hero-title-sub" style="display:block; font-size:1.15rem; color:#34D399; font-weight:600;">Precision Residential &amp; Commercial Cleaning &bull; <span class="city-name-text">${city}</span></span>
    `;
  }
